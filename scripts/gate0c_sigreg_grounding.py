...
from system1_jepa.sigreg import sigreg_epps_pulley
...
def main():
    ...
    ap.add_argument("--out", default="runs/of_jepa_navigate_sigreg.pt")
    args = ap.parse_args()
    dev = args.device
    ...
    for step in range(args.steps):
        ...
        sig = sigreg_epps_pulley(state.reshape(-1, state.shape[-1]))
        loss = args.jepa_weight * jepa + args.agent_weight * agent_loss + args.sigreg_weight * sig
        ...
        if (step + 1) % args.eval_every == 0:
            print(f"[gate0b] step {step+1}/{args.steps} jepa={float(jepa):.4g} "
                  f"agent_mse={float(agent_loss):.4f} sig={float(sig):.4f} held_out_rmse={held_out_rmse():.1f}px", flush=True)
    ...
    res = {"final_heldout_agent_rmse_px": round(final, 2), "precommit_px": 5.0,
           "gate0c_green": bool(final < 5.0)}
    print(json.dumps(res), flush=True)
    os.makedirs("artifacts", exist_ok=True)
    json.dump(res, open("artifacts/gate0c_sigreg_grounding.json", "w"), indent=2)
