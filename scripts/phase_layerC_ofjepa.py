"""Phase Layer-C with OF-JEPA binding: light up the per-slot CCT path.

Same pretrain+eval structure as phase_layerC_pretrain.py, but replaces
vanilla SlotAttention (which produced uniform binding mass — see
[[slot-attention-uniform-binding]]) with OFJEPAObjectFiles. OF-JEPA's
Sinkhorn-matched confidence per file IS the binding signal we need:
random-init probe shows confidence differentiates visible/hidden by
~26% (vs SlotAttention's ~0%), enough to drive the per-slot pipeline.

  pretrain: visible-only env → slot_predictor + SlotExistenceHead learn
            to predict the next-frame OF-JEPA confidence from slots+action
  eval:     freeze everything, switch to occluded env, measure
            surprise = disagreement between predicted next-confidence and
                       actual OF-JEPA confidence at t+1

Hypothesis (the validation test): per-slot disagreement surprise
spikes hidden > visible, revision_rate authorizes commitment revision
under occlusion. If this passes, the SlotExistenceHead path is no
longer inert.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Layer-C with OF-JEPA binding — per-slot CCT validator.")
    ...
    p.add_argument("--n-files", type=int, default=5,
                   help="agent + 3 targets + 1 buffer; lowering toward true n_entities sharpens Sinkhorn")
    p.add_argument("--slot-dim", type=int, default=64)
    p.add_argument("--ofjepa-checkpoint", type=str, default=None,
                   help="path to pretrained OF-JEPA state_dict (default: random init)")
...
def build_models(args, device):
    # OF-JEPA substrate provides slots + confidence; itself frozen (random
    # init is enough — per-frame Sinkhorn already differentiates visible
    # vs hidden by ~26% in mean confidence at init).
    substrate = OFJEPAObjectFiles(
        image_size=args.image_size, n_files=args.n_files,
        slot_dim=args.slot_dim, version="v0",
        checkpoint_path=args.ofjepa_checkpoint, device=str(device),
    )
    ...
    return substrate, predictor, relations, commit, existence, action_embed


def step_loop(
    *, args, env, substrate, predictor, relations, commit, existence,
    ...
    env.step(dxy)
    obs_now = env.observe()
    # OF-JEPA observation: returns slots + per-file confidence.
    ofb_now = substrate.observe(obs_now)
    slots_now = ofb_now.full_slot.detach()                # [B, N_files, slot_dim]
    ...
    # Use slots as both the world ledger AND the observation (OF-JEPA's
    # slots already encode the per-frame observation, so we pass them
    # twice — predictor will figure out how to use them).
    pred = predictor(slots_prev, slots_prev, act_emb)
    change_mask = pred["change_mask"]
    delta_metrics = slot_delta_loss(
        pred["next_slots"], slots_now,
    ...
    # Per-slot existence: predict next-frame confidence from predicted next slots.
    ...
    surprise_state["slots_prev"] = slots_now
...
def main() -> None:
    ...
    (substrate, predictor, relations, commit, existence, action_embed) = build_models(args, device)

    trainable = (
        list(predictor.parameters()) + list(relations.parameters())
        + list(commit.parameters()) + list(existence.parameters())
        + list(action_embed.parameters())
    )
    optim = torch.optim.AdamW(trainable, lr=args.lr)

    substrate.reset_episode(batch_size=args.batch_size)
    env_train.reset()
    obs0 = env_train.observe()
    ofb0 = substrate.observe(obs0)
    surprise_state = {
    ...
    for step in range(args.pretrain_steps):
        step_loop(
            args=args, env=env_train, substrate=substrate,
            predictor=predictor, relations=relations, commit=commit,
            existence=existence, action_embed=action_embed,
    ...
    # Freeze everything for eval.
    for m in (predictor, relations, commit, existence, action_embed):
    ...
    for step in range(args.eval_steps):
        rec = step_loop(
            args=args, env=env_eval, substrate=substrate,
            predictor=predictor, relations=relations, commit=commit,
            existence=existence, action_embed=action_embed,
            optim=None, surprise_state=eval_state, past_buffer=eval_past_buffer,
            training=False, phase="eval", step_offset=step, print_each=True,
        )
