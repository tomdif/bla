"""BC baseline on multi-target navigate. Phase 1.2.

Same architecture as bc_navigate.py — encoder + linear policy + expert
imitation — but on the harder MultiTargetNavigateEnv where the
visited-state is hidden from observation. Prediction: BC plateaus
because policy can't distinguish remaining vs visited targets, and
oscillates near targets it has already collected.
"""
...
@torch.no_grad()
def deploy(policy: BCPolicy, spec: MultiTargetNavigateSpec, n_episodes: int, device: torch.device, seed: int = 99) -> dict:
    policy.eval()
